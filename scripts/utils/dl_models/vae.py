...
tf.compat.v1.disable_eager_execution()  # Version compatibility fix
...
class VAE:
    ...
    def view_model_and_save(self):
        """
        Visualizes the models' architecture and saves a png in the models
        trained path: ../models/model_name/
        """
        dir_path = f'../models/{self.name}'
        directory = pathlib.Path(dir_path)
        if not directory.exists():
            os.makedirs(dir_path)
        visualkeras.layered_view(self.encoder, to_file=f'../models/{self.name}/{self.name}_encoder.png').show()
        visualkeras.layered_view(self.decoder, to_file=f'../models/{self.name}/{self.name}_decoder.png').show()
    ...
    def _add_dense_to_inf(self, encoder_inf_input):
        ...
        features = Embedding(50, 128)(encoder_inf_input)
    ...
    def _add_bottleneck(self, x, y):
        ...
        def sample_point_from_normal_distribution(args):
            """
            Create Gaussian distribution from 2 Dense layers

            :param args: MU and STD
            :return: Gaussian distribution sample point
            """
